...
def generate_images(prompt, width, height, guidance, iterations, seed, num_images, sr_scale, sr_aura):
    ...
    for i in range(num_images):
        filename = f"outputs/{slug}/it_{iterations}_guidance_{guidance}_sr_1_num_{i+1}.png"
        ...
        if sr_scale > 1:
            ...
            # Save upscaled image
            filename = f"outputs/{slug}/it_{iterations}_guidance_{guidance}_{'sr_' + str(sr_scale) if sr_scale else 'original'}_num_{i+1}.png"
            out.save(filename)
            print(f"Saved image: {filename}")
        elif sr_aura:
            print(f"Upscaling with aura...")
            out = apply_super_resolution_aura(out)
            filename = f"outputs/{slug}/it_{iterations}_guidance_{guidance}_{'sr_aura'}_num_{i+1}.png"
            out.save(filename)
...
def main():
    ...
    parser.add_argument("--sr-aura", type=bool, action="store_true", default=False, help="Super resolution with Aura")
